...
(defun edts-erlang-hook ()
 "Initialize EDTS."
  (setq erlang-man-root "~/.emacs.d/edts/doc/20.0")
  (require 'edts-start))
; }}}
...
(when (require 'erlang-start nil t)
  (when (not (null erlang-wrangler-path))
    ...
    (message "*** loading up wrangler libraries ***")
    (require 'wrangler))
...
(when (locate-library "distel")
  ...
  (defun distel-load-shell ()
    ...
    (setq erl-nodename-cache
          (make-symbol
           (concat
            "distel@" (car (split-string (shell-command-to-string "hostname -s"))))))
    ...
    (let ((file-buffer (current-buffer))
      ...
      (setq inferior-erlang-machine-options
        (list "-sname" "emacs" "-remsh" (string erl-nodename-cache))
                                        ; '("-name" "emacs@apple" "+P" "102400" "-setcookie" "emacs@sampath"
                                        ; "-pz" "ebin deps/*/ebin apps/*/ebin"
                                        ; "-boot" "start_sasl")
           )
        (message "** connecting to %s with options %s **" erl-nodename-cache inferior-erlang-machine-options)
